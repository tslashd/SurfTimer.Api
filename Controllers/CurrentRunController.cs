...
namespace SurfTimer.Api.Controllers
{
    ...
    public class CurrentRunController : ControllerBase
    {
        ...
        public async Task<ActionResult<PostResponseEntity>> InsertMapTime(
            [FromBody] MapTimeRunDataDto mapRunDataDto
        )
        {
            ...
            try
            {
                ...
                await _db.TransactionAsync(
                    async (conn, trx) =>
                    {
                        ...
                        {
                            Console.WriteLine(
                                $"Moving onto inserting {mapRunDataDto.Checkpoints.Count} checkpoints"
                            );
                            foreach (
                                var checkpointParams in from cp in mapRunDataDto.Checkpoints
                ...
                return CreatedAtAction(
                    nameof(InsertMapTime),
                    new { id = insertedMapTimeId },
        ...
        public async Task<ActionResult<PostResponseEntity>> UpdateMapTime(
            [FromBody] MapTimeRunDataDto mapRunDataDto,
            int mapTimeId
        )
        {
            ...
            try
            {
                ...
                await _db.TransactionAsync(
                    async (conn, trx) =>
                    {
                        ...
                        {
                            Console.WriteLine(
                                $"Moving onto updating {mapRunDataDto.Checkpoints.Count} checkpoints"
                            );
                            foreach (
                                var checkpointParams in from cp in mapRunDataDto.Checkpoints
                                let checkpointParams = new
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Failed to update MapTime and Checkpoints for ID {MapTimeID}.",
                    mapTimeId
                );
                return StatusCode(500, new { error = ex.Message });
            }
